...
def handle(cache):
    """
    Handle Key Metrics data operations
    ...
    while True:
        clear_screen()
        print_header("Key Metrics Data")
        
        options = [
            "Get Key Metrics for a Symbol",
            "View All Cached Key Metrics",
            "Export Key Metrics Data",
            "Return to Main Menu"
        ]
        ...
        if choice == 1:
            get_key_metrics(cache)
        elif choice == 2:
            view_all_key_metrics(cache)
        elif choice == 3:
            export_key_metrics(cache)
        elif choice == 4:
            break
        else:
...
def get_key_metrics(cache):
    """Get key metrics data for a specific symbol."""
    ...
    cached_data = cache.get_cached_data("metrics", symbol)
    
    if cached_data:
        print(f"\nFound cached key metrics data for {symbol}.")
        refresh = input("Do you want to refresh from API? (y/N): ").strip().lower()
        if refresh != 'y':
            display_key_metrics(cached_data, symbol)
            return
    ...
    print(f"\nFetching key metrics data for {symbol} from API...")
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            if not data:
                print(f"No key metrics data available for {symbol}.")
                return
            
            cache.track_api_request(endpoint)
            cache.save_data("metrics", symbol, data)
            display_key_metrics(data, symbol)
        else:
            print(f"API request failed with status code {response.status_code}")
    except Exception as e:
        print(f"Error fetching key metrics data: {str(e)}")

def display_key_metrics(data, symbol):
    """Display key metrics data in a readable format."""
    if not data:
    ...
    display_cols = ['date', 'peRatio', 'marketCap', 'dividendYield', 'roe']
    display_cols = [col for col in display_cols if col in df.columns]
    
    if not display_cols:
        print("No relevant key metrics data found.")
        return
    ...
    print(f"\nKey Metrics Data for {symbol}:")
    print(tabulate(display_df.head(10), headers="keys", tablefmt="pretty"))
    
    if len(df) > 10:
        print(f"\nShowing 10 of {len(df)} records. Export to view all data.")

def view_all_key_metrics(cache):
    """View a summary of all cached key metrics."""
    clear_screen()
    print_header("All Cached Key Metrics Data")
    ...
    if df.empty:
        print("No key metrics data cached yet.")
        return
    
    print(tabulate(df, headers="keys", tablefmt="pretty"))

def export_key_metrics(cache):
    """Export key metrics data to file."""
    from utils.export import export_data
    
    clear_screen()
    print_header("Export Key Metrics Data")
    
    cache.db.execute(
        "SELECT DISTINCT symbol FROM cache_data WHERE data_type = 'metrics'"
    )
    results = cache.db.fetchall()
    symbols = [row[0] for row in results] if results else []
    
    if not symbols:
        print("No key metrics data available to export.")
        return
    ...
    for symbol in symbols_to_export:
        cached_data = cache.get_cached_data("metrics", symbol)
        if cached_data:
            export_data(cached_data, f"{symbol}_key_metrics", "Key Metrics data")
